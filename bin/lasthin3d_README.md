...
sets the grid cell steps to '4.0'. 
...
only writes every 100th point.


    lasthin3d64 -i in.laz -random_nth 100 -o out.laz

only writes every 100th point on average. A random gernerator 
decides for each point if it is written.


    lasthin3d64 -i in.laz -random_nth 100 -seed 42 -o out.laz

seeds the random generator with '42'.


    lasthin3d64 -i in.laz -random_nth 100 -flag_as_keypoint -o out.laz

flags point as keypoint and does not remove any other points. 
The filter methods above can be specified with this flag and determine
which points are selected as keypoints.


    lasthin3d64 -i in.laz -random_nth 100 -flag_as_withheld -o out.laz

flags point as withheld and does not remove any other points. 
The filter methods above can be specified with this flag and determine
which points are selected as keypoints.
